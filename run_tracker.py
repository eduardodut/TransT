...
def track_sequences(sequences,net,ds):
    sequence_bbox_list = []
...
def track_sequence(model_pth,ds,annotation_label,min_confidence):
        ...
        backward_sequences = ds.loc[ds.bbox.apply(lambda x: len(x)>0)]
        backward_sequences['final_frame'] = backward_sequences['init_frame'].shift(1).fillna(-1).astype(int) + 1

        # backward_init_frame, backward_bbox_list = track_sequences(backward_sequences,net,ds)
        if forward_sequences.shape[0]>0:
            forward_init_frame, forward_bbox_list   = track_sequences(forward_sequences,net,ds)
        
        # out['backward_init_frame'] = backward_init_frame 
        # out['backward_bbox_list'] = backward_bbox_list 
            out['forward_init_frame'] = forward_init_frame 
            out['forward_bbox_list'] = forward_bbox_list 
        return out
def track_dataset(ds, dataset,model_pth,output_path,annotation_label,min_confidencemin_confidence):
    torch.cuda.empty_cache()
    out = track_sequence(model_pth,ds,annotation_label,min_confidence)
    if 'forward_init_frame' in out.keys():
        ds = ds.iloc[out['forward_init_frame']:]
        out_file = []
        for frame, bbox in zip(ds.itertuples(index=False),out['forward_bbox_list']):
            x1,y1,x2,y2 = bbox
...
def main(dataset_path, model_pth,annotation_label,min_confidence):
    ...
    for seq_label in image_project.samples.seq_label.unique():
        ...
        if not os.path.isfile(output_path):
            ...
            # track_dataset(ds, dataset,model_pth,output_path,annotation_label)
    pool = mp.Pool(2)
